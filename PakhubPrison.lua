--[[
    ...
    ██║     ██║  ██║██║  ██╗    ██║  ██║╚██████╔╝██████╔╝
    ╚═╝     ╚═╝  ╚═╝╚═╝  ╚═╝    ╚═╝  ╚═╝ ╚═════╝ ╚═════╝ 
        PAK HUB PRISON - SILENT AIM + FLY EDITION
]]
...
criarToggle(pageMovement, "Fly / Voo", "Voar (WASD move, Espaço sobe, Ctrl desce)", function(v) State.Fly = v end)
criarSlider(pageMovement, "Fly Speed", 20, 200, 60, function(v) State.FlySpeed = v end)
...
--// [SILENT AIM] VARIÁVEIS
local silentTarget = nil
local silentEnabled = false

--// [SILENT AIM] Acha o alvo mais próximo
...
--// [SILENT AIM] Hook protegido no __namecall
-- A função checkcaller() impede que o hook rode quando o PRÓPRIO jogo chama
-- Isso evita que o Hyperion (anti-cheat) detecte a modificação
local oldNamecall
...
--// [SILENT AIM] Hook protegido no __index (Mouse)
...
--// FLY / VOO - BodyVelocity + BodyGyro
local flyBV = nil
local flyBG = nil
local flyConn = nil
local flyTeclas = {W = false, A = false, S = false, D = false, Space = false, Ctrl = false, Shift = false}
...
local function iniciarFly()
    ...
    -- Cria BodyVelocity e BodyGyro para voo suave
    ...
    -- Conexão de input de teclado
    local UIS = UserInputService
    
    local function onInputBegan(input, process)
        if process then return end
        if input.KeyCode == Enum.KeyCode.W then flyTeclas.W = true end
        if input.KeyCode == Enum.KeyCode.A then flyTeclas.A = true end
        if input.KeyCode == Enum.KeyCode.S then flyTeclas.S = true end
        if input.KeyCode == Enum.KeyCode.D then flyTeclas.D = true end
        if input.KeyCode == Enum.KeyCode.Space then flyTeclas.Space = true end
        if input.KeyCode == Enum.KeyCode.LeftControl then flyTeclas.Ctrl = true end
        if input.KeyCode == Enum.KeyCode.LeftShift then flyTeclas.Shift = true end
    end
    
    local function onInputEnded(input)
        if input.KeyCode == Enum.KeyCode.W then flyTeclas.W = false end
        if input.KeyCode == Enum.KeyCode.A then flyTeclas.A = false end
        if input.KeyCode == Enum.KeyCode.S then flyTeclas.S = false end
        if input.KeyCode == Enum.KeyCode.D then flyTeclas.D = false end
        if input.KeyCode == Enum.KeyCode.Space then flyTeclas.Space = false end
        if input.KeyCode == Enum.KeyCode.LeftControl then flyTeclas.Ctrl = false end
        if input.KeyCode == Enum.KeyCode.LeftShift then flyTeclas.Shift = false end
    end
    
    local c1 = UIS.InputBegan:Connect(onInputBegan)
    local c2 = UIS.InputEnded:Connect(onInputEnded)
    
    -- Atualização do voo por frame
    local c3 = RunService.RenderStepped:Connect(function()
        ...
        -- Cálculo da direção baseado nas teclas
        if flyTeclas.W then direcao = direcao + camCF.LookVector end
        if flyTeclas.S then direcao = direcao - camCF.LookVector end
        if flyTeclas.A then direcao = direcao - camCF.RightVector end
        if flyTeclas.D then direcao = direcao + camCF.RightVector end
        if flyTeclas.Space then direcao = direcao + Vector3.new(0, 1, 0) end
        if flyTeclas.Ctrl then direcao = direcao - Vector3.new(0, 1, 0) end
        
        local velocidade = State.FlySpeed
        if flyTeclas.Shift then
            velocidade = velocidade * 2
        end
        
        flyBV.Velocity = direcao * velocidade
        flyBG.CFrame = camCF
    end)
...
-- Reconectar fly quando o personagem respawna
...
-- Controle de toggle do Fly
local oldFlyState = false

--// LOOP PRINCIPAL
RunService.RenderStepped:Connect(function()
    -- FLY: ativa/desativa quando o toggle muda
    if State.Fly ~= oldFlyState then
        oldFlyState = State.Fly
        if State.Fly then
            iniciarFly()
        else
            pararFly()
    ...
    -- Atualiza fly speed em tempo real
    if State.Fly and flyBV then
        -- já é atualizado no loop de fly
    end
